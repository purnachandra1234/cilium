...
func (ds *PolicyTestSuite) TestAddSearchDelete(c *C) {
	...
	repo.Mutex.RLock()
	c.Assert(repo.SearchRLocked(lbls2), DeepEquals, []*api.Rule{})
	repo.Mutex.RUnlock()
	...
	// search rule1,rule2
	repo.Mutex.RLock()
	c.Assert(repo.SearchRLocked(lbls1), DeepEquals, []*api.Rule{&rule1, &rule2})
	c.Assert(repo.SearchRLocked(lbls2), DeepEquals, []*api.Rule{&rule3})
	repo.Mutex.RUnlock()
