...
func (p *Repository) AllowsRLocked(ctx *SearchContext) api.Decision {
	...
	if len(p.rules) == 0 {
		ctx.PolicyTrace("No policy loaded\n")
	} else {
...
func (p *Repository) SearchRLocked(labels labels.LabelArray) []*api.Rule {
	result := []*api.Rule{}
...
// representation
func JSONMarshalRules(rules []*api.Rule) string {
