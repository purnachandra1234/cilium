...
func loadPolicyFile(path string) ([]api.Rule, error) {
	var content []byte
	var err error
...
func loadPolicy(name string) ([]api.Rule, error) {
	...
	if fi, err := os.Stat(name); err != nil {
		...
		return []api.Rule{}, fmt.Errorf("Error: %s is not a file or a directory", name)
	}
...
func processAllFilesFirst(name string, files []os.FileInfo) ([]api.Rule, error) {
	result := []api.Rule{}
...
func recursiveSearch(name string, files []os.FileInfo) ([]api.Rule, error) {
	result := []api.Rule{}
	for _, f := range files {
		if f.IsDir() {
